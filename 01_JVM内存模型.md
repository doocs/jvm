...
当方法运行过程中需要创建局部变量时，就将局部变量的值存入栈帧中的局部变量表中。方法运行结束后，该方法对应的栈帧出栈，并释放内存空间。

> 对于 Java，我们平时所说的栈，其实对应着 Java 栈帧中的局部变量表部分。而 Java 虚拟机栈，是由一个个栈帧组成的，每个栈帧都有：局部变量表、操作数栈、动态链接、方法出口信息等。
...
## 本地方法栈
本地方法是使用 Java 以外的**其他语言**编写的方法。

本地方法栈与 Java 虚拟机栈实现的功能类似，只不过本地方法栈是描述本地方法运行过程的内存模型。
